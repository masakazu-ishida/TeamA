...
public class RemoveFromCartCommitService {
	...
	public ItemsInCartDTO removeCart(String userId, int itemId) throws SQLException, ServletException {

		try (Connection conn = ConnectionUtil.getConnection(CommonConstants.LOOKUP_NAME)) {
			ItemsInCartDAO dao = new ItemsInCartDAO(conn);

			ItemsInCartDTO dto = dao.findById(userId, itemId);

			conn.setAutoCommit(false);

			try {
				dao.deleteId(userId, itemId);
