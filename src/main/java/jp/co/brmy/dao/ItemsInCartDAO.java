...
public class ItemsInCartDAO extends BaseDAO {
	...
	public ItemsInCartDTO findById(String userId, int itemId) throws SQLException {
		String sql = "select items_in_cart.user_id,items_in_cart.item_id,items_in_cart.amount,items_in_cart.booked_date,\n"
				+ "items.name,items.manufacturer,items.category_id,items.color,items.price,items.stock,items.recommended\n"
				+ "from items_in_cart inner join items on items_in_cart.item_id= items.item_id where items_in_cart.user_id=? and items_in_cart.item_id=?";
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setString(1, userId);
			ps.setInt(2, itemId);
	...
	public int deleteId(String userid, int itemId) throws SQLException {
